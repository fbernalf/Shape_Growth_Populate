...
function run!(model::CellModel{Dim}; num_steps::Int64 = 50) where Dim
    ...
    history_result, final_step = Shape_Growth_Populate.cellular_dynamics(
        model,num_steps)
...
function cellular_dynamics(
    ...
    for step in 1:num_steps
        ...
        new_cells = Shape_Growth_Populate.simulate_step!(
            model,
            current_cells
        )
        ...
        stromal_for_history = isnothing(model.stromal_cells) ? 
                          Dict{NTuple{Dim, Int64}, Shape_Growth_Populate.StromalCell{Dim}}() : 
...
function simulate_step!(
    model::CellModel{Dim},
    current_cells::Dict{NTuple{Dim, Int64}, Shape_Growth_Populate.Cell{Dim}}
) where Dim
    ...
    final_next_cells_dict = Dict{NTuple{Dim, Int64}, Shape_Growth_Populate.Cell{Dim}}()
...
function create_default_initial_cells_dict(::Val{Dim}, initial_cell_origin::NTuple{Dim, Int64}, initial_type::Int64 ) where Dim
    ...
    initial_cell = Shape_Growth_Populate.Cell{Dim}(
...
function create_default_initial_stromal_cells(::Val{Dim}, initial_stromal_cell_origin::NTuple{Dim, Int64}, initial_stromal_type::Int64 ) where Dim
    initial_stromal_cell = Shape_Growth_Populate.StromalCell{Dim}(
