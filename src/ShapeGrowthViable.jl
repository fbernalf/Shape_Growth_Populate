...
# Dans ShapeGrowthModels/src/ShapeGrowthViable.jl (ou functions.jl si c'est là que vous le mettez)

function is_near_stromal_cell(
    cell_to_check::ShapeGrowthModels.Cell{Dim},
    stromal_cells_dict::Dict{NTuple{Dim, Int64}, ShapeGrowthModels.Cell{Dim}}
) where Dim
    ...
    for (stromal_coord, _) in stromal_cells_dict
        ...
        if dist_sq <= ShapeGrowthModels.STROMAL_PROXIMITY_DISTANCE^2
